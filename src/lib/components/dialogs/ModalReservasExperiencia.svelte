...
{#if mostrar}
	<div
		class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-2 sm:p-4 animate-fade-in"
	>
		<div
			class="bg-neutral-900 border border-green-700 rounded-lg shadow-xl max-w-7xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col"
		>
			<!-- Header -->
			<div class="flex justify-between items-center p-4 sm:p-6 border-b border-green-700">
				<div>
					<h2 class="text-lg sm:text-2xl font-bold text-white flex items-center gap-2">📋 <span class="truncate">Reservas</span></h2>
			...
			<!-- Resumen -->
			<div class="p-4 sm:p-6 bg-neutral-800 border-b border-green-700">
				<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
					<div class="bg-neutral-900 rounded-lg p-4 border border-green-700/30">
						<div class="text-neutral-400 text-xs sm:text-sm mb-1">Total Reservas</div>
						<div class="text-2xl sm:text-3xl font-bold text-white">{totalReservas}</div>
					</div>
					<div class="bg-neutral-900 rounded-lg p-4 border border-green-700/30">
						<div class="text-neutral-400 text-xs sm:text-sm mb-1">Total Ingresos</div>
						<div class="text-xl sm:text-2xl font-bold text-green-400">
							{formatearMoneda(totalIngresos)}
						</div>
					</div>
					<div class="bg-neutral-900 rounded-lg p-4 border border-green-700/30">
						<div class="text-neutral-400 text-xs sm:text-sm mb-1">Total Personas</div>
			...
			<div class="px-4 pt-4 pb-2 sm:px-6">
				<div class="relative">
					<input
						...
						class="w-full px-4 py-2.5 sm:py-3 bg-neutral-800 border border-green-700/30 rounded-lg text-white placeholder-neutral-500 focus:outline-none focus:border-green-700 text-sm"
					/>
					{#if busqueda}
						<button
							onclick={() => (busqueda = '')}
			...
			<div class="flex-1 overflow-y-auto px-4 pb-4 sm:px-6 sm:pb-6">
				{#if reservas.length === 0}
					<div
						class="text-center py-12 text-neutral-500 border-2 border-dashed border-neutral-700 rounded-lg text-sm"
					>
						...
						{#each reservasFiltradas() as reserva}
							...
							>
								...
								<button
									...
									class="w-full px-4 py-3 flex items-center justify-between hover:bg-neutral-750 transition text-left"
								>
									<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 flex-1 min-w-0">
										<div class="flex items-center gap-2.5 min-w-0">
											<!-- Icono de expansión -->
											<span
												class="text-green-400 text-sm transform transition-transform duration-200 flex-shrink-0"
											...
											<div class="min-w-0">
												...
												<div class="flex items-center gap-1.5 text-xs sm:text-sm">
													{#if reserva.grupo}
														<span class="text-purple-400 font-medium">👥 Grupo</span>
														<span class="text-neutral-500">•</span>
														<span class="text-neutral-400">{reserva.cantidad_grupo || 0} pax</span>
													{:else}
														<span class="text-blue-400 font-medium">👤 Individual</span>
														<span class="text-neutral-500">•</span>
														<span class="text-neutral-400">{reserva.numero_cliente || 1} pax</span>
													{/if}
												</div>
											</div>
										</div>
